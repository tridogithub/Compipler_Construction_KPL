  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      do {
        ...
        checkFreshIdent(currentToken->string);
        ObjectNode *tmp = (ObjectNode*) calloc (1,sizeof(ObjectNode));
        tmp->object = createVariableObject(currentToken->string);
        tmp->next = varObj;
        varObj = tmp;
        check++;
      ...
      ObjectNode *tmp = varObj;

      while(tmp != NULL) {
        tmp->object->varAttrs->type = varType;
        declareObject(tmp->object);
        tmp = tmp->next;
      }
      eat(SB_SEMICOLON);
      varObj = NULL;
