...
Token *readNumber(void)
{
  ...
  if (isFloat == 0)
  {
    token->iValue = atoi(token->string);
  }
  ...
  if (isFloat > 1)
    error(ERR_INVALID_EXPRESSION, token->lineNo, token->colNo);
...
Token *getToken(void)
{
  ...
  case CHAR_RPAR:
    ...
    return token;
  case CHAR_INTER:
    token = makeToken(SB_INTER, lineNo, colNo);
    readChar();
    return token;
  case CHAR_LBRA:
    token = makeToken(SB_LBRA, lineNo, colNo);
    readChar();
    return token;

  case CHAR_RBRA:
    token = makeToken(SB_RBRA, lineNo, colNo);
    readChar();
    return token;
