...
void compileStatement(void) {
  ...
  case TK_IDENT:
    ...
    if(lookAhead->tokenType == SB_ASSIGN){
      ...
      compileAssignSubtractSt();
    }
    else if(lookAhead->tokenType == SB_LBRA){
      compileIndexes();
      compileAssignSt();
    }
    break;
...
void compileAssignDivideSt(void) {
  assert("Parsing an assign divide statement ....");
  eat(SB_ASSIGN_DIVIDE);
  compileExpression();
  assert("Assign divide statement parsed ....");
}

void compileAssignTimeSt(void) {
  assert("Parsing an assign times statement ....");
  eat(SB_ASSIGN_TIMES);
  compileExpression();
...
void compileAssignSubtractSt(void) {
  assert("Parsing an assign subtract statement ....");
...
void compileExpression3(void) {
  while (lookAhead->tokenType == SB_MINUS || lookAhead->tokenType == SB_PLUS || lookAhead->tokenType == SB_MOD)
  {
    switch (lookAhead->tokenType)
    {
    case SB_MINUS:
