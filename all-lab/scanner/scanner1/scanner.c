...
Token* readIdentKeyword(void) {
  ...
  while ((currentChar != EOF) && 
	  ((charCodes[currentChar] == CHAR_LETTER) || (charCodes[currentChar] == CHAR_DIGIT))) {
        if (count <= MAX_IDENT_LEN) token->string[count++] = (char)currentChar;
          readChar();           // continue read next char of this token
  }
  ...
  token->tokenType = checkKeyword(token->string);   // check if it is keyword , then assign tokentype

  if (token->tokenType == TK_NONE)// if token is not a keyword, then we know it is IDENT
