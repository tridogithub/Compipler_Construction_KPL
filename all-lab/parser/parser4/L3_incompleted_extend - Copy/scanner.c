...
Token* readNumber(void) {
  ...
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)) {
    if (charCodes[currentChar] == CHAR_PERIOD) flag++;
  ...
  if (flag > 1) { // none, invalid
    token->tokenType = TK_NONE;
    error(ERR_INVALIDNUMBER,lineNo, colNo);
  }

  token->string[count] = '\0'; // still integer
...
Token* getToken(void) {
  ...
  case CHAR_LBRA:
    ...
    cn = colNo;
    readChar();
    return makeToken(SB_LBRA,lineNo,colNo);
    
  case CHAR_RBRA:
    readChar();
    return makeToken(SB_RBRA, lineNo, colNo);
    
  case CHAR_PERCENT:
    readChar();
    return makeToken(SB_PERCENT,lineNo,colNo);

  case CHAR_INTER:
    readChar();
    return makeToken(SB_INTER,lineNo,colNo);
...
void printToken(Token *token) {
  ...
  case SB_ASSIGN_DIVIDE : printf("SB_ASSIGN_DIVIVDE\n"); break;
  ...
  case SB_LBRA: printf("SB_LBRA\n"); break;
  case SB_RBRA: printf("SB_RBRA\n"); break;
